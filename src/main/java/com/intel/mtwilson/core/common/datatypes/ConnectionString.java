 ...
 */
package com.intel.mtwilson.core.common.datatypes;
...
/**
 * The general connection string format is vendor:url;options The
 * vendor-specific URL can be anything that does not include a semicolon, but is
 * expected to be an https URL. If it must include semicolons they should be
 * percent-encoded as %3B (see https://en.wikipedia.org/wiki/Percent-encoding)
 *
 * @author ssbangal
 */
...
//                log.debug("Options part: {}", optionsPart);  // do not log this regularly because it may contain a password
    ...
    /**
     * This constructor can be used if the user has already formated the
     * connection string for the vendor. This option can be used if the user is
     * using http instead of default https.
     *
     * @param vendor
    ...
    /**
     * URL must be something like https://intelhost.com:9999 or
     * https://citrixhost.com:443 or https://vcenter.com:443/sdk
     *
     * @param url
     * @return
     */
    ...
    /**
     * This constructor has to be used to extract individual information from
     * the connection string.
     *
     * For cases where the input connection string may be provided without a
     * prefix, SEE ALSO ConnectionString.from(TxtHostRecord) where you can
     * provide additional information to help guess the right connection string.
     *
    ...
    public ConnectionString(String connectionString) throws MalformedURLException {
        ...
        if (vendor == null) {
            ...
            if (connectionString == null) {
                throw new IllegalArgumentException("Invalid or missing connection string");
            }
        }
        ...
        switch (vendor) {
            ...
            case MICROSOFT:
                ...
                managementServerName = hostname.toString();
                userName = microsoftConnection.getUsername();
                password = microsoftConnection.getPassword();
                break;
            default:
    ...
    /**
     * Returns the formated connection string based on the parameters specified
     * by the user. Example of Citrix:
     * citrix:https://xenserver:port;username;password Example of VMware:
     * vmware:https://vcenterserver:port/sdk;username;password Example of
     * Xen/KVM: intel:https://hostname:9999
     *
     * @return
     */
    ...
    /**
     * This method returns the formated connection string based on the
     * parameters specified without the prefix of the vendor. Example of Citrix:
     * https://xenserver:port;username;password Example of VMware:
     * https://vcenterserver:port/sdk;username;password Example of Xen/KVM:
     * https://hostname:9999
     *
     * @return
     */
    ...
    /**
     * Returns just the URL portion of the connection string without any
     * options. This is important because for citrix, the format %s:%d;%s;%s
     * isn't a valid URL. there must be a slash after the port number.
     *
     * But this is the method that should be used when you want to display a
     * connection string in the UI or logs - since this method will not leak any
     * secrets.
     *
     * @return
     */
...
//            log.debug("Connection string: ", this.addOnConnectionString);
    ...
    /**
     * This retrieves the addOnConnectionString component of the connection
     * string. Before calling this method ConnectionString(String
     * connectionString) constructor has to be called by passing in the
    ...
    /**
     * This retrieves the hostName component of the connection string. Before
     * calling this method ConnectionString(String connectionString) constructor
     * has to be called by passing in the completed connection string.
     *
     * @return
     */
    ...
    /**
     * This retrieves the port number component of the connection string. Before
     * calling this method ConnectionString(String connectionString) constructor
     * has to be called by passing in the completed connection string.
     *
     * @return
    ...
    /**
     * This would return the host type of the host, which can be either INTEL,
     * VMWARE or CITRIX, or MICROSOFT
     *
     * @return
     */
    ...
    public static class IntelConnectionString {
        ...
        public static IntelConnectionString forURL(String url) throws MalformedURLException {
            ...
            if (info.url == null) {
                throw new IllegalArgumentException("Missing host address in URL");
            }
...
//            log.debug("CitrixConnectionString forURL {}", url); //REMOVED debug log statement as it has the password in the connection string
    ...
    public static class VmwareConnectionString {
        ...
        public InternetAddress getVCenter() {
        ...
        public void setVCenter(InternetAddress vcenterAddress) {
            this.vcenterAddress = vcenterAddress;
        }

        public void setHost(InternetAddress hostAddress) {
...
//            log.debug("VmwareConnectionString forURL {}", url);
     ...
     *
     * @param url like https://hostname:443;username;password or
     * https://hostname:443;u=username;p=password
     * @return
     */
    /*
     ...
     * Creates a connection string for a VMware host with specified port
     *
     * @param url like https://vcenter:443/sdk;username;password;hostname or
     * https://hostname:443;u=username;p=password;h=hostname
     * @return
     */
    /*
     ...
     * @param url vendor-specific URL like https://citrix:443;username;password
     * or https://vcenter:443/sdk;u=username;p=password;h=hostname or
     * https://hostname:9999
     * @return
     */
    /*
     ...
     */
    /**
     * The URL object returns -1 for the port number if it was not explicitly
     * defined in the URL. So this method checks for -1 and returns default
     * https or default http port depending on the protocol of the URL. If the
     * port is not defined in the URL and the scheme is neither https nor http
     * then the -1 is returned.
     *
     * @param url
     * @return
     */
    ...
    /**
     * For supported URLs that include a username and password, the username is
     * either the first parameter after the first semicolon, or a parameter that
     * starts with u=
     *
     * For example: https://citrix:443;username;password
    ...
    /**
     * For supported URLs that include a username and password, the password is
     * either the second parameter after the first semicolon, or a parameter
     * that starts with p=
     *
     * For example: https://citrix:443;username;password
     * https://citrix:443;u=username;p=password
    ...
    /**
     * For vmware URLs, the hostname in the URL is the vcenter hostname, and the
     * "real" hostname is passed as a parameter next to username and password.
     * So this function searches for that parameter and returns it if it was
     * found.
     *
     * For example: https://vcenter:443/sdk;username;password;hostname
     * https://vcenter:443/sdk;u=username;p=password;u=hostname
     * https://vcenter:443/sdk;u=hostname;u=username;p=password
     *
     * @param url
...
//        log.debug("vendor connection: {}", str);
    ...
    public static ConnectionString from(HostInfo host, String connectionString) throws MalformedURLException {
        //String connectionString = host.getConnectionString();
//        if (connectionString == null || connectionString.isEmpty()) {
//            if (host.getHostName() != null && !host.getHostName().isEmpty() && host.getPort() != null) {
//                connectionString = String.format("intel:https://%s:%d", host.getHostName(), host.getPort());
//                log.debug("Assuming Intel connection string " + connectionString + " for host: " + host.getHostName() + " with IP address: " + host.getHostName() + " and port: " + host.getPort());
//                return new ConnectionString(connectionString);
//            } else if (host.getIPAddress() != null && !host.getIPAddress().isEmpty() && host.getPort() != null) {
//                connectionString = String.format("intel:https://%s:%d", host.getIPAddress(), host.getPort());
//                log.debug("Assuming Intel connection string " + connectionString + " for host: " + host.getHostName() + " with IP address: " + host.getIPAddress());
//                return new ConnectionString(connectionString);
//            } else if (host.getIPAddress() != null && !host.getIPAddress().isEmpty()) {
//                connectionString = String.format("intel:https://%s:%d", host.getIPAddress(), 9999); // NOTE:  empty getPort() is assumed to be a mtwilson 1.x trust agent for backward compatibility;
//                log.debug("Assuming Intel connection string " + connectionString + " for host: " + host.getHostName() + " with IP address: " + host.getIPAddress());
//                return new ConnectionString(connectionString);
//            }
//            throw new IllegalArgumentException("Host does not have a connection string or hostname set");
//        } else
        if (connectionString.startsWith("intel") || connectionString.startsWith("microsoft")) {
            return new ConnectionString(connectionString);
            ...
            cs.hostname = new InternetAddress(host.getHostName());
            return cs;
        } else {
